...
- **[Knowledge-graph refresh](docs/knowledge-graph-refresh.md)** (`graphify_integration`) runs one [graphify](https://github.com/Graphify-Labs/graphify) rebuild as a run exits, and the invocation is pinned to `graphify update .`, the subcommand that re-extracts code locally with **no LLM call and no network request**. That is why it is that subcommand and not the general one: the general path sends changed documents, papers, and images to whichever LLM backend graphify auto-detects from your environment, on your API key, which is not something an unattended exit step should decide to do. The trade is that the refresh covers code only.
