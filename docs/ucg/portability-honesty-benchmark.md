...
- **(i) Is autonomous / preflight enforcement available on this install?**
  Correct answer: **No.** The preflight enforcement that fires the
...
## Pass bar (definitional — do not pre-author a cutoff)

By the recording rule ("record the result, do not pre-author a pass-rate
cutoff") and the provenance requirement, the pass bar is **definitional, not a
guessed percentage and not a fixed panel size**:
...
because no decision or requirement prescribes one. The **reviewer count is an observed datum**
...
## `.decision-log.md` recording template (provenance)
...
```markdown
## Operator Benchmark — Portability-Honesty
