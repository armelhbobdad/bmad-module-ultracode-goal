...
description: Operator sign-off that each of the four planning shaping fragments encodes its workflow's FR-3 shift-left shaping as additive persistent_facts guardrails, signed against AD-9's note that the gate (INV-3), not the shaping, is the guarantee.
---

# UCG Awareness Fragment Shaping Decision

This is the story-1.4 decision-doc gate (AC-5). Content adequacy of a shaping
fragment is a human judgment, so for each of the four planning fragments an
operator confirms that every FR-3 shift-left shaping bullet for that workflow
appears as a `persistent_facts` guardrail entry, names the Epic-11 floor classes
the fragment targets, and records an `ACCEPTED` or `REWORK` verdict with a
reviewer and a date.

Every block below is signed against AD-9's downgrade note: these fragments are
run-scoped guardrail facts, narrower than per-AC machine directives by design.
The formalize/preflight gate (INV-3), not this shaping, is the load-bearing
guarantee that an artifact is UCG-ready. A fragment steers authoring toward
...
- `FR-3 shaping coverage:` and `Epic-11 floor classes:` lines,
...
- FR-3 shaping coverage: machine-checkable NFR budgets (named metric, concrete
...
- Epic-11 floor classes: invented-NFR-threshold (threshold numbers cite a source
...
- FR-3 shaping coverage: each load-bearing decision authored as an AD-n with an
  explicit Binds, Prevents, and Rule plus an ADOPTED marker once settled;
  Deferred-with-a-revisit-trigger entries instead of a silent TBD; resolve open
  questions as ADRs before preflight; AD ids stay stable and no inherited parent
  AD is silently weakened; no structural dimension the altitude owns is left
  wholly silent.
- Epic-11 floor classes: undecided-decision (unresolved or untagged forks an
...
- FR-3 shaping coverage: per-story gate-ability tag from {ci-machine,
...
- Epic-11 floor classes: vacuous AC; orphaned never-green index; leaked TEA
...
- FR-3 shaping coverage: each story inherits the epic AC contract verbatim
  ...
  Notes cite only real, resolvable source paths and real AD ids with no invented
  reference or guessed threshold; the story has a definable done derivable from
  its own ACs, verifications, and Tasks before ATDD handoff.
- Epic-11 floor classes: vacuous AC; orphaned never-green index; invented NFR
