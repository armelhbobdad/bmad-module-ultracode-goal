...
- **`project-context.md` count != 1** (`project_context_count`) — **non-budgeted advisory; never gates launch**: ensure exactly **one** `project-context.md` exists (generate via `bmad-generate-project-context` if zero; if multiple, this is ambiguous AI-rules state — resolve to one before launch). `{workflow.persistent_facts}` globs this file; duplicates poison Auto Memory grounding. Unlike the other items here, `preflight_check.py` reports `project_context_count` as a **check only** — it populates the `checks` dict and is **never appended to `blockers[]`** (the blocker list runs cc_version → framework → git-clean → protected-branch and stops; no project-context blocker is ever added), so `budget == 0` holds even with a **zero or duplicate** count. Remediate it to strengthen Auto-Memory grounding via `{workflow.persistent_facts}`, not to satisfy the step-4 `budget == 0` gate — a count of `0` (or `> 1`) never blocks launch.
