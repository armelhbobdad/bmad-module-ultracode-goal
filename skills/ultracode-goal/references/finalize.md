...
**Goal:** Make the run pay off for the next one. Capture what was learned to the right durable store, optionally run a retrospective, audit the decision log, produce a run report, and surface the deferred-work ledger. This is the terminal stage — reached by one of four routes: after the Epic-level gate advanced; when a story escalated and the run is `blocked`; when the run hit its `--max-stories` **work bound** with in-scope stories still remaining (`references/execute.md` — a `complete` outcome carrying no Epic-level gate, because the Epic is not finished and this stage authors none it did not earn); or when the run completed its in-scope *strict subset* of a deliberately-partial Epic without an Epic-level gate (a **`partial-complete`** outcome — attended only). The last two look alike and are not: `partial-complete` narrows **in-scope** and is attended-only, while the work bound leaves in-scope untouched and simply drives fewer of it, so it keeps the ordinary `complete` envelope. Headless never narrows in-scope, so a headless run that is *not* work-bounded still finishes the Epic and emits `complete`. Converse in `{communication_language}`; the run report and ledger are written in `{document_output_language}`.
