...
**The epic-level roll-up needs no hand-authoring at all where sprint-status is available: run it with `--rollup`** (see "Route on the verdict"), which computes the aggregate from the per-story artifacts this section already had you author — no `trace-<epic>.md` required. Hand-author the epic pair only where the gate cannot be given sprint-status. **In that fallback the roll-up needs the same hand-authoring, named for the EPIC.** Everything above is written per story, and on this stack TEA never runs — so nothing ever writes the epic's own `trace-<epic>.md`, while the omit-`--rollup` form of the gate below presumes it exists: `--story <epic_id>` in a per-story-named directory resolves nothing and fail-closes a completed Epic to `escalate`. When every story is `done`, hand-author the same two files named for the epic id — `trace-<epic>.md`, its frontmatter pointing at `gate-decision-<epic>.json` — with a `gate_status` that honestly summarises the per-story record: a PASS only when every story advanced, never a roll-up dressed over a story that did not. It takes no NFR and no test-review; the roll-up declares that omission with `--epic-level`, per "Route on the verdict" below.
...
- **`advance`** (gate_status PASS or WAIVED) → the story passes. **Sync the story's row in `sprint-status.yaml` to `done` before moving on** — set `development_status[<story_id>]` to `done`, preserving the file's comments and key order, and log the transition in `.decision-log.md`. This write is the run's own, not a sub-skill's: `bmad-dev-story` leaves the row at `review`, and `bmad-code-review` (production only — it does not run under `--light` at all) writes `done` **only** when nothing was left as an action item, so a story that legitimately `defer`s a non-critical finding is left at `in-progress`. Every downstream reader keys on `done` — Stage 1's in-scope rule (`references/ingest-and-scope.md` rule 3), the Epic-level trigger in this very sentence, and `scripts/drive_epic.py`'s progress check — so an advanced story whose row still says `review` is an advanced story the next reader re-drives, and an Epic whose last story advanced without this write never satisfies the Epic-level condition below. Sync it here, at the one place the authoritative verdict is known. Then move to the next story. When **every story of the Epic is `done`**, run the Epic-level gate: pass the epic's own id via `--story`, **add `--epic-level`**, and read its `gate_status` **only**. TEA produces NFR and test-review **per story, not per epic**, so there is no epic-level aggregate to AND, and every story's own production gate already ANDed its signals before reaching `done`. With `--rollup` the epic id is the sprint-status grouping key and the verdict is a computation over the per-story record; only in the omit-`--rollup` fallback is `--story` resolved against an epic-named artifact (scoped per the `--story` note above) and the roll-up a pure trace read.

```
uv run {skill-root}/scripts/gate_eval.py --trace-output {workflow.trace_output_dir} --story <epic_id> --profile production --epic-level --rollup --sprint-status {workflow.implementation_artifacts}/sprint-status.yaml
```

**`--rollup` computes the roll-up; it does not read an epic-named artifact.** The story set comes from `sprint-status.yaml` — the authority on what the Epic owes, never a typed list a story could quietly fall off — and the aggregate is the WORST per-story `gate_status`, fail-closed on five refusals, each `NOT_EVALUATED` → `escalate`: an unreadable sprint-status; an epic with no stories in it; any story not `done` (the roll-up's own trigger is every-story-done, so running it early is refused rather than answered); a multi-story epic whose trace dir is not per-story-named (one shared artifact cannot decide N stories); and any `done` story with no resolvable gate artifact of its own — Stage 1's `done-without-gate-artifacts` audit made binding at the one gate that certifies the Epic. The sprint-status key is the resolution identity, byte-identical to the artifact `<id>` exactly as the `--story` rule above pins it — a renamed or slugged mismatch fail-closes, and the remedy is naming, never slug tolerance. Omit `--rollup` and the invocation reads the epic's own `trace-<epic>` artifact exactly as before; prefer `--rollup` wherever sprint-status is available, because the epic summary then rests on a computation over the per-story record instead of on prose nobody re-checks. **The partial-by-design exception below binds `--rollup` unchanged**: a deliberately partial Epic authors no epic-level gate and runs no roll-up — and one run anyway is answered by the not-`done` refusal, never by a PASS over a subset.
