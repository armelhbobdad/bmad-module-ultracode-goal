# Benchmark label — epic-judgment-seed (story 2.3 AC-5)

Labelled DEFECT fixture for the step-3 second-hypothesis-stream benchmark. Reuses the Epic-11
`invented_nfr_threshold` floor class (INV-5): an NFR threshold number with no cited source and not
marked UNKNOWN.
...
- **Expected seeded-subagent result (AC-5 oracle):** the seeded throwaway subagent CONFIRMS this
...
the matched `reds[]` source to the run's `.decision-log.md` (AD-5 / NFR-9 channel). Operator-benchmark
half is parked to the end-of-epic batch review.
