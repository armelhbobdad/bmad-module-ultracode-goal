"""Measurement-protocol doc-shape, tracked files only.

The wall-clock-ceiling source lives in the gitignored planning corpus; it is verified
manually at authoring time and recorded in the run's .decision-log.md, NOT CI-gated here —
a test must not depend on a gitignored path. CI asserts only the tracked surface: the
SKILL/preflight measurement prose and the absence of any authored ceiling number /
time-cutoff. The timing-block behavior + no-time-based-block are covered in
test_formalize_check.py (in-process, monkeypatched delay). Stdlib + pytest.
"""
...
def test_ac2_measurement_protocol_documented():
    ...
    # no new telemetry sink prose (reuses the existing decision-log channel only)
    ...
    # anti-vacuous: a stale block dropping the channel reference would fail the channel assertion
