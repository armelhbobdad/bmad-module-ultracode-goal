...
function runStory(s, i) {
  ...
  return agent(
    `You are delivering ONE story of BMAD Epic "${epic}" inside an ISOLATED git worktree (isolation: worktree). ` +
      ...
      `0. RECORD THIS STORY'S BASELINE, before any implementation and before step 1 touches a file: write the full 40-hex output of \`git rev-parse HEAD\` for THIS worktree's branch tip to ${implArtifacts}/.baseline-${id}, as one newline-terminated line. It must be this worktree's own HEAD — each worktree branches independently — and the long 40-hex form, not --short: the PreToolUse hook compares it as an exact string with no normalization. Write it once and never refresh it; the story keeps the SHA it started from even after HEAD moves.\n` +
      ...
      `3. Run and PRINT the project's test, lint, and build commands. In a monorepo, scope the test run wide enough to catch cross-package regressions — run the full suite, or the changed package(s) plus their dependents (e.g. --affected and downstream), not just this story's own package: a story's change can regress a sibling package's conformance gate within this worktree, and a package-scoped run reports green while that sibling gate is red. Once all three are green, write the tests-ran marker file ${implArtifacts}/.tests-ran-${id} (exact name the PreToolUse hook checks, with ULTRACODE_STORY_ID=${id} for this worktree) so the hook permits the commit. That marker MUST contain a \`baseline=<sha>\` line whose value is the verbatim, whitespace-stripped content of ${implArtifacts}/.baseline-${id} from step 0 — copied, never re-derived from \`git rev-parse HEAD\` at marker-write time. The hook compares the two strings character for character and denies the commit when they differ or when the line is missing; you are several steps of work past step 0 by now, so a re-derived value would read a HEAD that has already moved and deny your own commit. Do NOT proceed to commit unless all three are green.\n` +
      reviewStep +
      `5. COMMIT AT GREEN: make exactly ONE commit on this worktree's branch capturing the story. Stage this story's paths in a prior, SEPARATE tool call (git add <this story's paths>), then run git commit as a second, distinct tool call. "git commit -a" and a chained "git add ... && git commit" are unsupported here: the hook pre-evaluates the command string, so the chained add has not run yet and the staged index is still empty. The PreToolUse hook will deny the commit on a protected branch, without the tests-ran marker, or with an empty staged index — that is expected enforcement, satisfy it rather than bypass it. Set committed=true only if the commit succeeded on green.\n` +
...
const epicGate = await agent(
  `All stories of BMAD Epic "${epic}" have been delivered on their worktree branches. ` +
    `Run the EPIC-LEVEL TRACE gate over the consolidated epic — a trace-only read of the epic's gate_status, declaring --epic-level so the production NFR/test-review ANDs are skipped (the per-story ANDs already ran at each story gate, and TEA writes no epic-level aggregate; a placeholder epic-level --test-review path would fail-closed and spuriously downgrade an epic PASS to reloop). Declare it with the flag, never by omitting --nfr/--test-review: under --profile production an omitted flag is itself a failing signal.` +
    ...
    `, then run EXACTLY:\n` +
    `  uv run ${skillRoot}/scripts/gate_eval.py --trace-output ${traceOutput} --story ${epic} --profile ${profile} --epic-level\n` +
