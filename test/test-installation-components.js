...
/**
 * The fixture-tracking validator must actually FAIL on an ignored fixture.
 *
 * A validator that only ever passes proves nothing, and this one guards a silent
 * failure mode: `git add` on an ignored path is a no-op, so the fixture stays on
 * disk (local suite green) while the commit never carries it (CI red). The proof
 * is a real ignored file, created and removed inside the test.
 */
async function testFixtureTrackingValidator() {
  console.log(`${colors.yellow}Testing fixture-tracking validator...${colors.reset}`);

  const { execFileSync } = require('node:child_process');
  const repoRoot = path.resolve(__dirname, '..');
  const validator = path.join(repoRoot, 'tools', 'validate-fixtures.js');

  const run = () => {
    try {
      execFileSync('node', [validator, '--strict'], { cwd: repoRoot, encoding: 'utf8' });
      return 0;
    } catch (error) {
      return error.status;
    }
  ...
  assert(await fs.pathExists(validator), 'validate-fixtures.js exists');

  // Pole 1: the tree as shipped is clean.
  assert(run() === 0, 'validator passes on the shipped fixture tree');

  // Pole 2: a genuinely ignored fixture makes it fail. `.claude/` is ignored
  // repo-wide with no fixture-tree negation, and a fixture mimicking a project's
  // .claude dir is exactly the shape that would slip through unnoticed.
  const probeDir = path.join(repoRoot, 'skills/ultracode-goal/scripts/tests/fixtures/_validator_probe/.claude');
  let failedAsExpected = false;
  try {
    await fs.ensureDir(probeDir);
    await fs.writeFile(path.join(probeDir, 'settings.json'), '{}\n', 'utf8');
    failedAsExpected = run() === 1;
  } finally {
    await fs.remove(path.join(repoRoot, 'skills/ultracode-goal/scripts/tests/fixtures/_validator_probe'));
  }
  assert(failedAsExpected, 'validator exits 1 on an ignored fixture');

  // Pole 3: and it recovers, so the probe left nothing behind.
  assert(run() === 0, 'validator passes again once the ignored fixture is gone');

  // Generated bytecode inside the fixture tree is correctly ignored and must NOT
  // be reported, or the check cries wolf on every machine that ran the suite.
  const pycacheDir = path.join(repoRoot, 'skills/ultracode-goal/scripts/tests/fixtures/_validator_probe_pyc/__pycache__');
  let toleratedGenerated = false;
  try {
    await fs.ensureDir(pycacheDir);
    await fs.writeFile(path.join(pycacheDir, 'x.cpython-312.pyc'), 'x', 'utf8');
    toleratedGenerated = run() === 0;
  } finally {
    await fs.remove(path.join(repoRoot, 'skills/ultracode-goal/scripts/tests/fixtures/_validator_probe_pyc'));
  }
  assert(toleratedGenerated, 'validator ignores generated bytecode inside the fixture tree');
...
async function runTests() {
  ...
  await testFixtureTrackingValidator();
