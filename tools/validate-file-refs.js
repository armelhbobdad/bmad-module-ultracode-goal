...
// `agents/` is where bmad-method core agents land under `_bmad/`; `scripts/` is
// BMAD core's install-time tooling (e.g. resolve_customization.py); `custom/` is
// the user-side customization override folder. None are sourced from this repo.
...
function mapInstalledToSource(refPath) {
  ...
  // _bmad/ucg/ → skills/ (UCG module)
  if (cleaned.startsWith('ucg/')) {
    return path.join(SRC_DIR, cleaned.slice('ucg/'.length));
  }
...
function resolveRef(ref) {
  ...
  if (ref.type === 'workflow-data') {
    ...
    //  1. Bare path starting with a workflow name (e.g., 'ultracode-goal/references/foo.md')
